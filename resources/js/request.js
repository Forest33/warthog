function query() {
    ...
    astilectron.sendMessage(req, function (message) {
        ...
        $('#badge-result').html("0: OK").css('visibility', 'visible').addClass('bg-success');
        $('#time-spent').html(message.payload.data.spent_time)
        $('#query-result').html(syntaxHighlight(message.payload.data.json_string))
        showHeadersTrailers(message.payload.data.header, message.payload.data.trailer)
    });
}

function getRequestData(field, root) {
    let data = {}
    if (root === undefined) {
            ...
            if (field.map !== undefined) {  // todo валидация дублей ключей
                if (field.map.fields === undefined) {
                    ...
                    root.find('.request-repeated-message[data-repeated-fqn="' + field.fqn + '"]').each(function () {
                        let key = $(this).find('[data-map-key="' + field.fqn + '"]').val()
                        items[key] = $(this).find('[data-map-value="' + field.fqn + '"]').val()
                    })
                    if (Object.keys(items).length > 0) {
                        data[field.fqn] = items
                    }
                } else {
                    let objects = {}
                    root.find('.request-repeated-message[data-repeated-fqn="' + field.fqn + '"]').each(function () {
                ...
                if (messageValues.length > 0) {
                    if (!field.repeated) {
                        data[field.fqn] = messageValues[0]
                    } else {
                        data[field.fqn] = messageValues
                    }
                }
            }
            break
...
function showQueryError(err) {
    ...
    $('#time-spent').html('')
    $('#badge-result').html(err.code + ": " + err.code_description).css('visibility', 'visible').removeClass('bg-success').addClass('bg-danger');
...
function showHeadersTrailers(header, trailer) {
    ...
    if (header !== null && Object.keys(header).length !== 0) {
        $('#query-result-headers').html(syntaxHighlight(JSON.stringify(header, null, 1)))
        $('#nav-result-headers .header').css('visibility', 'visible')
    }
    if (trailer !== null && Object.keys(trailer).length !== 0) {
        $('#query-result-trailers').html(syntaxHighlight(JSON.stringify(trailer, null, 1)))
...
function setQueryRunButton() {
    $('#request-run').removeClass('btn-danger bi-stop-fill').addClass('btn-success bi-play-fill')
}

function setQueryCancelButton() {
    $('#request-run').removeClass('btn-success bi-play-fill').addClass('btn-danger bi-stop-fill')
}
