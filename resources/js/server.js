...
function loadServer(srv, show) {
    ...
    astilectron.sendMessage(req, function (message) {
        ...
        if (message.payload.data.services.length > 0) {
                    ...
                    service.methods.forEach(function (method, m_idx) {
                        ...
                        if (showServiceIdx === s_idx) {
                            selMethods.append($('<option>', {
                                value: method.name,
                                text: method.name,
                                selected: (show !== undefined && show.method.name === method.name) || (show === undefined && m_idx === 0),
                            }))
                        }
                    })
                }
            ...
            if (show === undefined && !isNull(message.payload.data.services[0].methods) && message.payload.data.services[0].methods.length > 0) {
                createRequestForm(message.payload.data.services[0], message.payload.data.services[0].methods[0])
                return
            }

            if (!isNull(show) && currentServices[show.service.name] !== undefined && !isNull(currentServices[show.service.name].methods) &&
                currentServices[show.service.name].methods[show.method.name] !== undefined) {
                createRequestForm(currentServices[show.service.name], currentServices[show.service.name].methods[show.method.name])
            } else if (message.payload.length > 0 && !isNull(message.payload[0].methods) && message.payload[0].methods.length > 0) {
                createRequestForm(message.payload.data.services[0], message.payload.data.services[0].methods[0])
            }
        }
    });
...
function createRequestForm(service, method) {
    ...
    if (isNull(currentQuery) && !isNull(currentServer.data.request) && !isNull(currentServer.data.request[service.name]) &&
    ...
    for (const f of fields) {
        if (currentRequest !== undefined && currentRequest[f.field.fqn] !== undefined) {
    ...
    switch (field.type) {
        case protoTypeEnum:
            if (!field.repeated) {
                ...
                field.enum.values.forEach(function (item, idx) {
                    sel.append($('<option>', {value: item.number, text: item.name, selected: idx === 0}))
                })
            } else {
                tmpl = getRepeatedInput(field, 'request-select-input', field.enum.value_type, field.name, currentInputCount)
            }
            break
        case protoTypeBool:
            if (!field.repeated) {
                ...
                tmpl = getRepeatedInput(field, 'request-bool-input', field.type, field.name, currentInputCount)
            }
            break
        case protoTypeMessage:
            ...
            btn.click(function () {
                ...
                let attr = {'data-input-fqn': field.fqn, 'data-input-id': currentInputCount}
                for (let key in attr) {
                ...
                $(inputDelete.find('.request-message-button-delete')[0]).click(function () {
                    ...
                    $(this).closest('.request-message-container').attr('data-container-input-count', inputCount)
                ...
                if (field.map !== undefined) {
                    ...
                    let repeated = $('<div class="request-repeated-message request-input" data-input-id="' + currentInputCount + '" data-repeated-fqn="' + field.fqn + '">')
                    if (field.map.fields === undefined) {
                        ...
                        repeated.append(getFieldTemplate({
                            type: field.map.key_type,
                            name: 'key',
                            'data-map-key': field.fqn
                        }, attr))
        ...
        default:
            if (!field.repeated) {
                ...
                tmpl = getRepeatedInput(field, 'request-text-input', field.type, field.name, currentInputCount)
            }
    }
...
function getRepeatedInput(field, inputTmpl, inputType, inputName, dataID) {
    ...
    btn.click(function () {
        ...
        $(inputDelete.find('.request-message-button-delete')[0]).click(function () {
            ...
            $(this).closest('.request-message-container').attr('data-container-input-count', inputCount)
            $('.request-input[data-input-id="' + id + '"]').remove()
            buttonAdd.attr('data-input-count', parseInt(buttonAdd.attr('data-input-count')) - 1)
        })

        let input = $(template[inputTmpl])
        $(input.find('.label-name')[0]).attr('for', field.fqn + "." + currentInputCount).html(inputName)
        ...
        $(input.find('.field-value')[0]).attr('id', field.fqn + "." + currentInputCount)
        for (let key in attr) {
...
function saveRequest() {
    if (!isNull(currentQuery) || isNull(currentServer) || isNull(currentService) || isNull(currentMethod)) {
        return
    }
    ...
    if (isNull(currentServer.data.request) || isNull(currentServer.data.request)) {
        currentServer.data.request = {}
    }
    if (isNull(currentServer.data.request[currentService.name]) || isNull(currentServer.data.request[currentService.name])) {
...
function setRequestData(field, tmpl, data) {
    if (field.oneof !== undefined) {
        tmpl.find('.field-value[data-oneof-fqn="' + field.oneof.fqn + '"]').val(field.fqn).change()
    }

    switch (field.type) {
        case protoTypeEnum:
            if (!field.repeated) {
                $(tmpl.find('.field-value[data-field-fqn="' + field.fqn + '"] option[value="' + data + '"]')[0]).attr('selected', 'selected')
            } else {
                let btn = tmpl.find('.request-message-button-add[data-button-fqn="' + field.fqn + '"]')
                data.forEach(function (d, idx) {
                    btn.trigger('click')
                    $(tmpl.find('.field-value[data-field-fqn="' + field.fqn + '"] option[value="' + d + '"]')[idx]).attr('selected', 'selected')
                })
            }
            break
        case protoTypeBool:
            if (!field.repeated) {
                $(tmpl.find('.field-value[data-field-fqn="' + field.fqn + '"]')[0]).prop('checked', data)
            } else {
                let btn = tmpl.find('.request-message-button-add[data-button-fqn="' + field.fqn + '"]')
                data.forEach(function (d, idx) {
                    btn.trigger('click')
                    $(tmpl.find('.field-value[data-field-fqn="' + field.fqn + '"]')[idx]).prop('checked', d)
                })
            }
            break
        case protoTypeMessage:
            ...
            if (field.map !== undefined) {
                if (field.map.fields === undefined) {
                    ...
                    for (let key in data) {
                        btn.trigger('click')
                        let container = $(tmpl.find('.request-repeated-message[data-repeated-fqn="' + field.fqn + '"]')[idx++])
                        container.find('.field-value[data-map-key="' + field.fqn + '"]').val(key)
                        let f = {
                            fqn: field.fqn + '-map-value',
                            type: field.map.value_type,
                        }
        ...
        default:
            if (!field.repeated) {
                $(tmpl.find('.field-value[data-field-fqn="' + field.fqn + '"]')[0]).val(data)
            } else {
                let btn = tmpl.find('.request-message-button-add[data-button-fqn="' + field.fqn + '"]')
                data.forEach(function (d, idx) {
                    btn.trigger('click')
                    $(tmpl.find('.field-value[data-field-fqn="' + field.fqn + '"]')[idx]).val(d)
                })
            }
    }
}
