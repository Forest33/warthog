...
import {currentQuery, currentServer, loadServer, saveRequest,} from "./server.js";
import {dataIdToNode, isNull, showModalError, treeNodeNames, treeRootNodes} from "./index.js";
...
function initTreeDrag() {
        ...
        if (targetNode.data.type === sourceNode.data.type) {
            if (
                ...
                if (targetNode.state.expanded) {
                    ...
                    list = [sourceNode].concat(!isNull(targetNode.nodes) ? targetNode.nodes : []);
                } else {
    ...
    function canDrop(targetNode) {
        ...
        if (!isNull(treeNodeNames.get(targetNode.data.id)) && treeNodeNames.get(targetNode.data.id).has(sourceNode.title)) {
            return false;
        }
        if (sourceNode.data.type !== WorkspaceTypeFolder && !isNull(targetNode.data.parent_id) && treeNodeNames.get(targetNode.data.parent_id).has(sourceNode.title)) {
            return false;
        }
...
function onTreeNodeRender(node) {
    ...
    if (!treeNodeNames.has(node.data.parent_id)) {
        treeNodeNames.set(node.data.parent_id, new Map());
    }
    treeNodeNames.get(node.data.parent_id).set(node.title);
