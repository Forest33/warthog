function initTreeDrag() {
    ...
    $('#tree .list-group-item').hover(function () {
    ...
    function handleDrop(e) {
        ...
        if (targetNode.data.type === sourceNode.data.type) {
            if (sourceNode.data.type === WorkspaceTypeServer || sourceNode.data.type === WorkspaceTypeQuery) {
        ...
        } else if (sourceNode.data.type === WorkspaceTypeQuery && targetNode.data.type === WorkspaceTypeServer) {
            list = insertAfter(getParent(sourceNode).nodes, sourceNode, null)
        } else if (sourceNode.data.type === WorkspaceTypeServer && targetNode.data.type === WorkspaceTypeFolder) {
            if (targetNode.state.expanded) {
                list = insertAfter(getParent(sourceNode).nodes, sourceNode, null)
            } else {
    ...
    function canDrop(targetNode) {
        ...
        if (sourceNode.data.type === WorkspaceTypeQuery && sourceNode.data.parent_id === targetNode.data.id) {
            return true
        }
        if (sourceNode.data.type === WorkspaceTypeQuery && targetNode.data.parent_id !== sourceNode.data.parent_id) {
            return false
        }
        if (sourceNode.data.type === WorkspaceTypeFolder && targetNode.data.type !== WorkspaceTypeFolder) {
            return false
        }
        if (sourceNode.data.type === WorkspaceTypeServer && targetNode.data.type === WorkspaceTypeQuery) {
            return false
        }
        return true
    }
...
function getTreeDropdown(node) {
    let menu = $('<ul class="dropdown-menu" aria-labelledby="tree-dropdown-' + node.data.id + '"></ul>')
    ...
    switch (node.data.type) {
        case WorkspaceTypeFolder:
            menu.append($('<li><a class="dropdown-item" data-id="' + node.data.id + '" data-action="create-folder"><i class="bi bi-file-plus"></i> Add folder</a></li>'))
            menu.append($('<li><a class="dropdown-item" data-id="' + node.data.id + '" data-action="create-server"><i class="bi bi-server"></i> Add server</a></li>'))
            menu.append($('<li><a class="dropdown-item" data-id="' + node.data.id + '" data-action="rename-folder"><i class="bi bi-pen"></i> Rename</a></li>'))
            menu.append($('<li><a class="dropdown-item' + ext + '" data-id="' + node.data.id + '" data-action="delete"><i class="bi bi-trash"></i> Delete</a></li>'))
            break
        case WorkspaceTypeServer:
            menu.append($('<li><a class="dropdown-item" data-id="' + node.data.id + '" data-action="edit-server"><i class="bi bi-pen"></i> Edit</a></li>'))
            menu.append($('<li><a class="dropdown-item' + ext + '" data-id="' + node.data.id + '" data-action="delete"><i class="bi bi-trash"></i> Delete</a></li>'))
            break
        case WorkspaceTypeQuery:
            menu.append($('<li><a class="dropdown-item" data-id="' + node.data.id + '" data-action="delete"><i class="bi bi-trash"></i> Delete</a></li>'))
            break
    }

    return '<span class="bi bi-three-dots tree-node-dots" data-bs-toggle="dropdown" aria-expanded="false" id="tree-dropdown-' + node.data.id + '"></span>' + menu.prop('outerHTML')
}
...
function treeMenuCreateFolder(node) {
    ...
    $(input).insertAfter($('#tree .list-group-item[data-nodeid="' + node.nodeId + '"]'))
...
function treeFolderHandler(node, isCreate) {
    $('#tree-folder-form').submit(function (event) {
